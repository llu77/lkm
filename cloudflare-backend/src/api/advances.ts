...
app.get('/list', async (c) => {
  try {
    ...
    let query = `SELECT * FROM advances WHERE 1=1`;
    const bindings: any[] = [];
...
app.post('/create', async (c) => {
  try {
    ...
    if (!branchId || !employeeId || !amount || !month || !year) {
      return c.json<ApiResponse>({
        success: false,
    ...
    const result = await c.env.DB.prepare(
      `INSERT INTO advances (
        ...
        amount, month, year, description, recorded_by
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      RETURNING *`
    )
      .bind(
...
app.delete('/delete/:id', async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(
      `DELETE FROM advances WHERE id = ? RETURNING id`
    )
      .bind(id)
      .first();
